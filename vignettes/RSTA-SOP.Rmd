...
author: "Christian Gunge Riberholt, Joachim Birch Milan, Markus Harboe Olsen"
output: 
...
In `RTSA` the alpha level is set in the function by the argument alpha. For a signifance level of 5%, which is also the default value, set:
```{r, eval = FALSE}
RTSA(alpha = 0.05, ...)
```
...
For traditional meta-analysis it has been custom to use the inconsistency ($I^2$) for calculating heterogeneity [2]. This can also be used in the TSA, but it is recommended to calculate and adjust for diversity ($D^2$) [10]. Diversity is a more conservative estimate for heterogeneity affecting the required information size if heterogeneity is present. The TSA will then present a diversity adjusted required information size (DARIS) [10]. For meta-analysis where the required information size is not reached and a 0% heterogeneity is found, a predefined subgroup analysis adjusting for diversity between 25% to 50% could be relevant. Including more future trials will increase the risk of an increase in heterogeneity. This should then be clearly predefined in the protocol.

In `RTSA` the meta-analysis is per default adjusted by the estimated diversity, if the meta-analysis is a retrospective meta-analysis. If one wants to explore different levels of diversity, say 50%, the following can be achieved by:
```{r, eval = FALSE}
...
In `RTSA` for relative risks can both the argument `RRR` be used or the argument `mc` can be used. For a relative risk reduction of 20%, set `RRR = 0.2` or `mc = 0.8` (RR of 80%):
```{r, eval = FALSE}
RTSA(RRR = 0.2, ...)
RTSA(mc = 0.8, ...)
```
