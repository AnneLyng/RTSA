...
```{r}
mp = RTSA:::metaPrepare(outcome = "RR", data = perioOxy, method = "IV",
                        alpha = 0.05)
```
...
```{r}
sm = RTSA:::synthesize(mp, sign = NULL, fixedStudy = FALSE, 
                       hksj = FALSE, tau.ci.method = "BJ") # RTSA function
...
```{r}
metaanalysis(outcome = "RR", data = perioOxy, study = perioOxy$study, 
             method = "IV", tau.ci.method = "QP")
ma <- metaanalysis(outcome = "RR", data = perioOxy, study = perioOxy$study, 
             method = "IV", tau.ci.method = "BJ")
ma$synthesize$ci.tau
ma <- metaanalysis(outcome = "RR", data = perioOxy, study = perioOxy$study, 
             method = "IV", tau.ci.method = "QP")
ma$synthesize$ci.tau
```
...
```{r}
library(meta)
metagen(log(mp$te), mp$sig, method.tau.ci = "BJ")
metagen(log(mp$te), mp$sig, method.tau.ci = "QP")
```
...
boundaries <- RTSA:::boundary(inf_frac = trials[,1],
                 side = 2, alpha = 0.05, zninf = -8, tol = 1e-14)
boundaries
```
...
If we want a function that combines the metaPrepare, synthesize and TSA function, then we can use the RTSA function. This function prints the outcome of the TSA function without the need to specify the metaPrepare function, as it is embedded in the RTSA function.

```{r}
RTSA(data = perioOxy, outcome = "RR", mc = 0.9, side = 2, alpha = 0.05)
```
...
```{r}
RTSAout <- RTSA(data = perioOxy, outcome = "RR", mc = 0.7, side = 2, alpha = 0.05,
                fixed = FALSE)
plot(RTSAout)
```

```{r}
RTSAout <- RTSA(data = coronary, outcome = "RR", mc = 0.8, side = 2, alpha = 0.05)
plot(RTSAout)
```
...
```{r}
data("coronary")
mp = RTSA:::metaPrepare(outcome = "OR", data = perioOxy, method = "MH",
                        alpha = 0.05)
synout = RTSA:::synthesize(mp, hksj = TRUE, sign = NULL, 
                          fixedStudy = FALSE, tau.ci.method = "BJ")
synout
```
