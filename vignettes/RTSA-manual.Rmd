...
This manual will start with a short introduction to when a sequential meta-analysis should be considered before moving to main section of this manual about the statistical background. The statistical background section will contain a short introduction to hypothesis testing, type-1- and type-2-errors, ...

## Why do a sequential meta-analysis

Testing the same hypothesis more than once, using standard meta-analysis methods, as data/studies accumulate sequentially over time is known to inflate the risk of finding false positives. Inflation of finding false positives is also called inflation of the type-1-error. Sequential meta-analysis controls the risk of finding false positives by using group sequential methods originally created for clinical trials.
...
Each of these will be explained below. For all of the different hypothesis tests, are we interested in the distinction between the null hypothesis, often denoted $H_0$, and the alternative hypothesis, often denoted $H_A$. The statistical hypothesis test is from the perspective of finding evidence against the null hypothesis. Hence we will believe in the null hypothesis until proven otherwise in relation to some threshold, which in TSA is the pre-specified $\alpha$ significance level. A subsection is dedicated to describe the role of $\alpha$. For now we will return to the different form of hypothesis tests available in `RTSA`. 

We will use some mathematical notation when presenting the different types of hypothesis tests. Let $\mu$ be the effect estimated from data of interest, which can includes mean difference, risk ratio (relative risk), odds ratio or risk difference. We often wish to compare the effect estimated to a value described by the null hypothesis, we define this value to be $\mu_0$. For mean differences $\mu_0$ is often 0, while for risk and odds ratios the value is 1. 
