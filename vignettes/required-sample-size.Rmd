...
This vignette will explore the different methods for calculating the required sample size and the required number of trials to achieve a specific power in a meta-analysis. All methods are implemented in the `RTSA` package. The sample size calculations are intended for meta-analyses and not single studies. 
...
We will be considering heterogeneity a lot in this vignette. When heterogeneity is expected or present, we need to adjust our meta-analysis to accommodate this extra source of variation. When there is heterogeneity present in a meta-analysis, we use either $\tau^2$, $I^2$ (inconsistency) or $D^2$ (diversity) to try an quantify the level of heterogeneity. These metrics are however quite difficult to estimate in most meta-analyses and are often very uncertain. This is especially true for small meta-analyses, which do not include many studies. Heterogeneity has an impact on both the interpretation of the meta-analysis results and on the power calculation as described in @Ioannidis2007. It is strongly recommended that this uncertainty must be reported and taken into consideration when both interpreting the results of the meta-analysis and when conducting the sample size calculation. The `RTSA` package includes confidence intervals and standard errors on the measures of heterogeneity: $\tau^2, I^2$ and $Q^2$. These intervals calculated via the `confint.rma.uni()` function from the `metafor` package in `R`.

We will see in this vignette how to incorporate the estimation uncertainty when doing a sample size calculation. The original sample size calculation in TSA did not incorporate the uncertainty.

We start however with considering scenarios without heterogeneity. 

# Prospective meta-analysis

When the sample size calculation isn't based on an existing meta-analysis, we are considering a prospective meta-analysis. This section is about the power of yet to be conducted meta-analyses. We are investigating how many participants and trials we need for a prospective meta-analysis. 

To use a current meta-analysis for calculating how many more participants and trials are needed for achieving a specific level of power is investigated in the Retrospective section \@ref(retro). We recommend that one reads the prospective section before looking at the retrospective section.  

## No heterogeneity

For meta-analyses without heterogeneity, we will be fitting fixed-effect meta-analysis models, whereas we will be fitting random-effects meta-analysis models for meta-analyses with heterogeneity. 
...
What we need to define in order to make the sample size calculation differs depending on whether our outcome is risk ratios (RR), odds ratios (OR), risk differences (RD) or mean differences (cont). For mean differences, we need specification of: 

- The minimum clinically relevant difference $\delta$.
- Its expected standard deviation $s.e.(\delta)$
...
1. Is the current meta-analysis underpowered?
...
## Current power of meta-analysis

We can calculate the current power of the meta-analysis by defining the minimal clinically relevant value called `delta` in the `metaanalysis` function. We use the formulas presented in @Jackson2017 to calculate the current power for respectively a fixed- and random-effects meta-analysis.
...
Here we find that we additionally need 22836 participants over 22 trials to have sufficient power given the estimate of $\tau^2$. 

If we take into consideration the uncertainty of the $\tau^2$ estimate, we can compute the additional number of participants and trials given the lower and upper limit of the $\tau^2$ estimate. For the lower limit of $\tau^2$, we have that we need at minimum 3 more trials with 5034 particpants per trial:
```{r}
ma$out_ris$NR_bc
```
If we increase the number of additional trials to 6, we need 1049 per trial.
