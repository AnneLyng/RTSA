...
#' @param sign TBA
#' @param random TRUE or FALSE. Whether a random-effects model estimate and belonging statistics should be calculated. Defaults to TRUE.
...
#' @param hakn TRUE or FALSE. When conducting a random-effects model should the Harting-Knapp-Sidik-Jonkman adjustment be used. Else is DerSimonian-Laird used. Defaults to FALSE.
#'
  ...
  if(y$method == "GLM"){
    ...
    if(random == TRUE){
      ...
      if(fixedStudy == FALSE){
        ...
        glmRandomFit <- lme4::glmer(grpOut ~ -1 + eff + trial + (sv-1|trial), family = binomial,
                              nAGQ = 7)
      }
