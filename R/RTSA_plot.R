...
#' @export
#'
#' @importFrom ggplot2 ggplot coord_cartesian geom_hline theme_bw geom_vline geom_line geom_point aes theme element_blank geom_ribbon xlab ylab scale_x_continuous expansion scale_y_continuous scale_fill_identity scale_colour_manual ggtitle geom_segment geom_label scale_y_reverse sec_axis theme_classic element_text margin
#'
#' @examples
#' data(perioOxy)
#' outRTSA <- RTSA(data = perioOxy, outcome = "RR", mc = 0.7, side = 2, alpha = 0.05,
#' beta = 0.2, fixed = FALSE)
#' plot(x = outRTSA)
#'
plot.RTSA = function(x, ...){
 ...
 p <- ggplot(data = bounddat) +
    coord_cartesian(xlim = c(0,max(bounddat$inf_frac,1.1)),
